...
working_folder = 'FB_debug_delay_1us_Ig_0505_2.5'
home = os.getcwd()
...
nCav = np.array([14])
h = np.array([7560])
RoQ = np.array([73])*nCav
delay_time = np.array([1e-6]) # in unit of second
delay = [int(delay_time*f0*h[0])*N_bins] # number of data points corresponding to the delay time.
...
Vref_I = np.array([52675398.062246])
    ...
    for thetaL_factor in range(N_thetaL):
        for delay_factor in range(N_delay):
            for gain_factor in range(N_gain):
                ...
                delay = [int(delay_time*f0*h[0])*N_bins]
                print("delay : ",delay)
