...
working_folder = 'eSR/Haipeng_6D6F'
home = os.getcwd()
...
QL = np.array([329321.8554911,329321.8554911])
Vref_I = np.array([52675398.062246,52675398.062246])
Vref_Q = np.array([-2370878.834453,-2370878.834453])
Iref_I = np.array([0.4869135928499,0.4869135928499])
Iref_Q = np.array([-0.02191560337399,-0.02191560337399])
I_I_ref_ini = np.array([0.243456796425,0.243456796425])
I_I_ref_final = np.array([0.486913592849,0.486913592849])
I_Q_ref_ini = np.array([-0.0109578016869,-0.0109578016869])
I_Q_ref_final = np.array([-0.02191560337399,-0.02191560337399])
detune = np.array([0.0,0])
detune_ini = np.array([0.0,0])
detune_mid = np.array([-9970.53659004,-9970.53659004])
detune_final = np.array([-19941.0731801,-19941.0731801])
...
if nRF == 1:
    Qs = np.sqrt(h[int(mainRF)]*atomicZ*np.abs(Vref_I[int(0)])*eta/(2*np.pi*Ek))
elif nRF != 1 :
    Qs = np.sqrt(h[int(mainRF)]*atomicZ*np.abs(Vref_I[int(0)]+Vref_I[1])*eta/(2*np.pi*Ek))
...
for charge_factor in range(N_samples):
    for thetaL_factor in range(N_thetaL):
        ...
        if nRF == 2 :
            #for Haipeng's proposal
            
            VnewF = 2.25e6 # per cavity
            VnewD = 1.6e6
            PhisPhasor[0] = 81.23/180*pi
            PhisPhasor[1] = -81.23/180*pi
            
            Vs[0] = VnewF*np.cos(PhisPhasor[0])
            Vq[0] = VnewF*np.sin(PhisPhasor[0])
            Vs[1] = VnewD*np.cos(PhisPhasor[1])
            Vq[1] = VnewD*np.sin(PhisPhasor[1])
            
            #Vnew = 5.8583e6 # per cavity
            #PhisPhasor =87.415/180*pi
            #Vnew = 5.863e6 # per cavity
            #PhisPhasor = 87.1863201469/180*pi
