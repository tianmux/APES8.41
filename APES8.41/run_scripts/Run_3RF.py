...
pi = np.pi
clight = 299792458
working_folder = '3rd_strong_energy_FB'
home = os.getcwd()
...
ThetaL_min[0] = 20 # for the first RF system, focusing one
ThetaL_max[0] = 20 

ThetaL_min[1] = 0 # for the second RF system, defocusing one
ThetaL_max[1] = 0

ThetaL_min[2] = 0 # for the second RF system, defocusing one
ThetaL_max[2] = 0
...
for charge_factor in range(N_samples):
    for thetaL_factor in range(N_thetaL):
        ...
        nCav = np.array([9,5,4]) # number of fundamental cavities
        h = np.array([7560,7560,22680]) 
    ...
    # for fundamental, 
        ...
        V3rdtot = 1.88e6 #per cavity
        phis3rd = (-3.5-90)/180*pi
        
        Vs[2] = V3rdtot*np.cos(phis3rd)
        Vq[2] = V3rdtot*np.sin(phis3rd)
