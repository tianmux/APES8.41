...
working_folder = '/media/dataHD/APES_sim_data/EIC/eSR/591_gap_FB'
home = os.getcwd()
...
delay_time = np.array([0,0]) # in unit of second
delay = np.array([int(delay_time[i]*f0*h[0])*N_bins for i in range(len(delay_time))]) # number of data points corresponding to the delay time.

n_fb_on = np.array([1000,1000])
gII = np.array([1.0,1])
gQQ = np.array([1.0,1])
gIQ = np.array([0.0,0])
gQI = np.array([0.0,0])
...
iMin = 2.5
iMax = 2.5 
nParMin = iMin/f0/n_bunches/qe
nParMax = iMax/f0/n_bunches/qe
...
Prad0 = Prad
# setup the delay scan
N_delay = 1
delay_min = 1e-6 
delay_max = 1e-6
dDelay = (delay_max-delay_min)/N_delay
# setup the gain scan
N_gain = 20 
gain_min = 0
gain_max = 10 
dGain = (gain_max-gain_min)/N_gain
...
for charge_factor in range(N_samples):
    for thetaL_factor in range(N_thetaL):
        for delay_factor in range(N_delay):
            for gain_factor in range(N_gain):
                delay_time[0] = delay_min+dDelay*delay_factor
                delay = np.array([int(delay_time[i]*f0*h[0])*N_bins for i in range(len(delay_time))])
                print("delay : ",delay)
                for iRF in range(nRF):
                    gII[iRF] = gain_min+dGain*gain_factor
                    gQQ[iRF] = gain_min+dGain*gain_factor
                nPar0 = NperBunch
                ...
                Urad0 = Prad/(n_bunches*nPar*qe*f0) # radiation caused Voltage total, depends on beam kinetic energy
                print(Prad,',',(n_bunches*nPar*qe*f0))
                U_loss = Urad0/NC 
                print("Urad0 : ",Urad0)
                ...
                if nRF == 2 :
                    Vnew = 5861889.40948 # per cavity
                    PhisPhasor = 87.31406235913148/180*pi
                    Vs[0] = Vnew*np.cos(PhisPhasor)
                    Vq[0] = Vnew*np.sin(PhisPhasor)
                    Vs[1] = Vs[0]#(Urad0-Vs[0]*NF)/ND
                    Vq[1] = -Vq[0]#(Vquard_need*(NF+ND)-Vq[0]*NF)/ND
                if nRF == 1:
        ...
        # please leave the one that is going be used uncommented, and comment out the other two.
                ...
                #args = ("../APESAVX2")
                #args = ("../APESGCC")
                args = ("~/Dropbox/code/Cpp/APES_pack/APES8.4/run512.sh")
                print(cwd)
                ...
                path = os.path.join(cwd,"{0:03d}".format(charge_factor)+"{0:03d}".format(thetaL_factor)+"{0:03d}".format(delay_factor)+"{0:03d}".format(gain_factor)+"nmacro{0:.0f}".format(Npar)+"_nBin{0:.0f}".format(N_bins)+"_Idc{0:.2f}A".format(n_bunches*nPar*f0*qe)+"_ThetaL{0:.1f}degree".format(180/pi*thetaL[0])+"_delay{0:.3f}us".format(delay_time[0]/1e-6)+"_gain{0:.2f}_{1:.2f}".format(gII[0],gQQ[0]))
